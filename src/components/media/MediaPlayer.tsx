 ...
 * Media elements request in CORS mode (the CDN allows any origin, and a
 * non-opaque response is not subject to Chrome's Opaque Response Blocking)
 * and without a Referer (document policy `same-origin`, set in the layout):
 * objkt's CDN hotlink-protects against foreign referrers.
 */
  ...
  return (
    <div className="absolute inset-0">
      ...
      >
        ...
        <span className="t-label absolute bottom-6 left-1/2 -translate-x-1/2 text-white/80">
          {label}
        </span>
      </button>
    </div>
    ...
    <div className="absolute inset-0">
      ...
      {gw.src && !gw.exhausted ? (
        <audio
          ...
          autoPlay
          preload="none"
          className="absolute bottom-6 left-1/2 w-[min(90%,480px)] -translate-x-1/2"
        />
      ) : (
        <UnavailableNote />
      )}
...
function UnavailableNote() {
  return (
    <p className="t-label absolute bottom-6 left-1/2 -translate-x-1/2 bg-black/70 px-3 py-1.5 text-white/80">
...
export function MediaPlayer({ media }: { media: Media }) {
  switch (media.kind) {
    ...
    case "image":
      ...
      return isAnimatedMime(media.mime) && media.original.length ? (
        <OriginalImage media={media} />
      ) : (
        <MediaImage media={media} sizes="100vw" fit="contain" priority />
      );
    default:
      return <MediaImage media={media} sizes="100vw" fit="contain" />;
  }
